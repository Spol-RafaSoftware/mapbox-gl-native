...
NativeMapView::NativeMapView(jni::JNIEnv& _env,
    ...
    // Create a renderer
    auto renderer = std::make_unique<Renderer>(pixelRatio, *threadPool, fileSource,
                                               MapMode::Continuous, GLContextMode::Unique,
                                               jni::Make<std::string>(_env, _programCacheDir));

    // Create a renderer frontend
    rendererFrontend = std::make_unique<AndroidRendererFrontend>(std::move(renderer), *this,
